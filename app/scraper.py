...
from app.models import Obituary, DistinctObituary, db # Updated import: from app.models
from geopy.geocoders import Nominatim
# from app import scraping_active  <- Removed, if this was used, need to find its new location
...
# Configure logging (already configured in __init__.py, but keeping it here for now, can remove if needed)
logging.basicConfig(
    level=logging.INFO,
...
def process_city(session, subdomain, stop_event, db_session): # Added db_session here
...
def process_obituary(session, db_session, url, visited_obituaries, stop_event): # Modified to accept db_session
    ...
    try:
        ...
        if is_alumni:
            ...
            if not distinct_entry_exists:
                ...
                db_session.add(distinct_obituary_entry)
                db_session.commit() # Commit here to save DistinctObituary
...
def main(stop_event, db_session): # Modified to accept db_session
    ...
    for idx, subdomain in enumerate(subdomains, 1):
        ...
        process_city(session, subdomain, stop_event, db_session) # Pass db_session here
